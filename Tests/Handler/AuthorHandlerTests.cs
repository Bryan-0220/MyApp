...
namespace Tests.Handler
{
    public class AuthorCommandHandler
    {
        ...
        public async Task DeleteAuthor_ShouldFailValidation(string id)
        {
            var service = A.Fake<Application.Authors.Services.IAuthorService>();
            var validator = new DeleteAuthorCommandValidator();
            var handler = new DeleteAuthorCommandHandler(service, validator);
            var input = new DeleteAuthorCommandInput { Id = id };
            await Assert.ThrowsAsync<FluentValidation.ValidationException>(() => handler.Handle(input, CancellationToken.None));
        }
        ...
        {
            var service = A.Fake<Application.Authors.Services.IAuthorService>();
            var handler = new DeleteAuthorCommandHandler(service, A.Fake<FluentValidation.IValidator<DeleteAuthorCommandInput>>());

            A.CallTo(() => service.DeleteAuthor("non-existent", A<CancellationToken>._))
                .Returns(Task.FromResult(Result<Author>.Fail("Author not found")));

            var output = await handler.Handle(new DeleteAuthorCommandInput { Id = "non-existent" }, CancellationToken.None);

            Assert.False(output.Success);
            Assert.Equal("Author not found", output.Message);
            Assert.Null(output.AuthorId);
        }

        [Fact]
        public async Task DeleteAuthor_ShouldReturnSuccess_WhenAuthorDeleted()
        {
            var service = A.Fake<Application.Authors.Services.IAuthorService>();
            var handler = new DeleteAuthorCommandHandler(service, A.Fake<FluentValidation.IValidator<DeleteAuthorCommandInput>>());
            ...
            A.CallTo(() => service.DeleteAuthor("author-1", A<CancellationToken>._))
                .Returns(Task.FromResult(Result<Author>.Ok(author, "Author deleted")));

            var output = await handler.Handle(new DeleteAuthorCommandInput { Id = "author-1" }, CancellationToken.None);

            Assert.True(output.Success);
            Assert.Equal("Author deleted", output.Message);
            Assert.Equal("author-1", output.AuthorId);

            A.CallTo(() => service.DeleteAuthor("author-1", A<CancellationToken>._)).MustHaveHappenedOnceExactly();
        }
    }
}
